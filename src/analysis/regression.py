...
for row_id in range(2, max_row_2+1): 
    tuple_2.append(([x.strip() for x in str(sheet_2.cell(row=row_id, column=2).value).split(';')]))#, datetime.strptime(str(sheet.cell(row=row_id, column=1).value), '%Y/%m/%d')))
...
other_year = 0
year_old = datetime.strptime(sheet_2.cell(row=0 + 2, column=1).value, '%Y/%m/%d').year
year_new = 0
islam_year = 0
dict = {'year': [], "islam": []}
for row_id in range(0, max_row_2-2):     
    ...
    if (datetime.strptime(sheet_2.cell(row=row_id + 2, column=1).value, '%Y/%m/%d').year > year_old):
        year_new = datetime.strptime(sheet_2.cell(row=row_id + 2, column=1).value, '%Y/%m/%d').year
